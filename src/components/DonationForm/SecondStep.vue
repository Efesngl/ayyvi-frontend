<template>
  <div class="row justify-content-center justify-content-md-end p-2 pt-md-3 pe-md-5">
    <div class="col-12 col-md-4 p-3 bg-white text-center" id="donate-form">
      ...
      <!-- <h4>Lütfen kart bilgilerinizi giriniz</h4> -->
      <div class="row">
        <div class="col-12 p-3 text-start">
          ...
          <input type="number" class="form-control" @keydown.space.prevent v-model="donationInfo.card.cardCvv" id="card-cvv" name="card-cvv" />
          ...
          <div class="row" v-if="isCustomAmountShowed">
            <div class="col-12">
              <input type="text" class="form-control mt-2" @keyup="customAmount" name="other-price" id="other-price" placeholder="Örnek: 500" />
            </div>
          </div>
          <div class="row">
  ...
  methods: {
    ...
    makeDonate() {
      if (this.checkErrors()) {
        this.$axios
          .post("donate", {
            donationAmount: this.donationInfo.amount,
            userID: useUserStore().ID,
            cc: this.donationInfo.card,
          })
          .then((res) => {
            console.log(res);
          ...
          .catch((error) => {
            if (error.response.status == 400) {
              this.$swal.fire({
                ...
                title: "Lütfen kart bilgilerinizi kontrol ediniz !",
              });
            }
          });
      }
